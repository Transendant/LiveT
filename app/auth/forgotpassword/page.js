  ...
  const forgotPasswordHandler = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        process.env.NEXT_PUBLIC_API_BASE  + "/api/auth/forgotpassword",
        { email },
        config
      );
    ...
    <>
      ...
      <section className="lg:pt-[92px] pt-[82px] lg:h-screen px-6 bg-[#1b1a1f] relative w-full flex justify-center items-center">
        <form
          ...
          className="py-20 flex gap-8 justify-center items-center w-full"
        >
          {/* LEFT OF THE FORM */}
          <div className="flex flex-col w-full max-w-[360px]">
            {/* LOGIN */}
            <h6 className="text-[24px] font-bold mb-8 text-[#f2f2f2]">Forgot Password</h6>
            {/* INPUT FIELDS + CTA */}
